...
if ( !class_exists("Freeio_Mobile_Menu") ) {
    class Freeio_Mobile_Menu extends Walker_Nav_Menu {
       ...
       /**
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            $show_menu = true;
            if ( !empty($item->apus_user_role) ) {
                $show_menu = false;
                if ( in_array('', $item->apus_user_role) ) {
                    $show_menu = true;
                } else {
                    if ( is_user_logged_in() ) {
                        $user = wp_get_current_user();
                        $roles = ( array ) $user->roles;
                        foreach ($item->apus_user_role as $role) {
            ...
            if ( !$show_menu ) {
                ...
                if ( isset($item->apus_text_label) && !empty($item->apus_text_label) ) {
                    switch ( $item->apus_text_label ) {
                        case 'label_new':
                            $text_label  = esc_html__('New', 'freeio');
                            break;

                        case 'label_hot':
                            $text_label  = esc_html__('Hot', 'freeio');
                            break;

                        case 'label_featured':
                            $text_label  = esc_html__('Featured', 'freeio');
                            break;
        ...
        public function generateSubMegaMenu( $item, $apus_mega_profile ) {
            if ( $apus_mega_profile ) {
                ...
                if ( $post_ids && isset($post_ids[0]) ) {
                    $post_id = $post_ids[0];
                    $post_id = freeio_get_lang_post_id($post_id, 'apus_megamenu');
                    $post = get_post($post_id);
                    
                    $content = apply_filters( 'freeio_generate_post_builder', do_shortcode( $post->post_content ), $post, $post->ID);

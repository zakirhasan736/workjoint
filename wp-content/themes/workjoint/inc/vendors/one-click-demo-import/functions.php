...
function freeio_ocdi_import_files() {
    $demos = array();
    $demos[] = array(
        ...
        'import_notice'                => esc_html__( 'Import process may take 5-10 minutes. If you facing any issues please contact our support.', 'freeio' ),
        'preview_url'                  => 'https://demoapus1.com/freeio/',
    );
    return apply_filters( 'freeio_ocdi_files_args', $demos );
}
add_filter( 'pt-ocdi/import_files', 'freeio_ocdi_import_files' );

function freeio_ocdi_after_import_setup( $selected_import ) {
    ...
    $freelancer_menu   = get_term_by( 'name', 'Freelancer', 'nav_menu' );

    set_theme_mod( 'nav_menu_locations', array(
            'primary' => $main_menu->term_id,
            'mobile-primary' => $mobile_main_menu->term_id,
            'employer-menu' => $employer_menu->term_id,
            'freelancer-menu' => $freelancer_menu->term_id,
        )
    );
    ...
    if ( file_exists($file) ) {
        freeio_ocdi_import_settings($file);
    }
...
add_action( 'pt-ocdi/after_import', 'freeio_ocdi_after_import_setup' );

function freeio_ocdi_import_settings($file) {
    ...
    if ( !empty($datas['page_options']) ) {
        freeio_ocdi_import_page_options($datas['page_options']);
    }
    if ( !empty($datas['metadata']) ) {
        freeio_ocdi_import_some_metadatas($datas['metadata']);
    }
}

function freeio_ocdi_import_page_options($datas) {
    if ( $datas ) {
...
function freeio_ocdi_import_some_metadatas($datas) {
    if ( $datas ) {
...
function freeio_ocdi_before_widgets_import() {
...
add_action( 'pt-ocdi/before_widgets_import', 'freeio_ocdi_before_widgets_import' );
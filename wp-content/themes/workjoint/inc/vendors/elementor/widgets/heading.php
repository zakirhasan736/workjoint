 ...
 * @since 1.0.0
 */
class Freeio_Elementor_Widget_Heading extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Apus Heading', 'freeio' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => esc_html__( 'Add Your Heading Text Here', 'freeio' ),
			]
		);
		...
		$this->add_control(
            ...
            [
                'name' => 'image',
                'label' => esc_html__( 'Line Decor Image', 'freeio' ),
                'type' => Controls_Manager::MEDIA,
                'placeholder'   => esc_html__( 'Decor Background Image', 'freeio' ),
            ]
        );
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .freeio-heading-title' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .freeio-heading-title',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .freeio-heading-title' => 'mix-blend-mode: {{VALUE}}',
				],
				'separator' => 'none',
			]
		);
	...
	protected function render() {
		...
		$this->add_render_attribute( 'title', 'class', 'freeio-heading-title' );
        ...
        if ( !empty($settings['img_src']['id']) ) {
            $title_html .= freeio_get_attachment_thumbnail($settings['img_src']['id'], 'full');
        }
	...
	protected function _content_template() {
		...
		view.addRenderAttribute( 'title', 'class', [ 'freeio-heading-title', 'freeio-size-' + settings.size ] );
...
if ( version_compare(ELEMENTOR_VERSION, '3.5.0', '<') ) {
    Plugin::instance()->widgets_manager->register_widget_type( new Freeio_Elementor_Widget_Heading );
} else {
    Plugin::instance()->widgets_manager->register( new Freeio_Elementor_Widget_Heading );
}
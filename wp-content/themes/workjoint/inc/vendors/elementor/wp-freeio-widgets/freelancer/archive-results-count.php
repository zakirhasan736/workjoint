...
class Freeio_Elementor_Widget_Freelancer_Archive_Results_Count extends Elementor\Widget_Base {

	public function get_name() {
		return 'apus_element_freelancer_archive_results_count';
	}

	public function get_title() {
		return esc_html__( 'Freelancer Archive:: Results Count', 'freeio' );
	}
	...
	protected function render() {
        ...
        if ( !empty($freeio_freelancers) ) {
	        $total = $freeio_freelancers->found_posts;
			$per_page = $freeio_freelancers->query_vars['posts_per_page'];
			$current = max( 1, $freeio_freelancers->get( 'paged', 1 ) );
		} else {
...
Elementor\Plugin::instance()->widgets_manager->register( new Freeio_Elementor_Widget_Freelancer_Archive_Results_Count );

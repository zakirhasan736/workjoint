...
class Freeio_Elementor_Widget_Detail_Freelancer_Related extends Elementor\Widget_Base {

	public function get_name() {
		return 'apus_element_detail_freelancer_related';
	}

	public function get_title() {
		return esc_html__( 'Freelancer Details:: Related', 'freeio' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
            ...
            [
                ...
                'input_type' => 'number',
                'description' => esc_html__( 'Limit freelancers to display', 'freeio' ),
                'default' => 4
            ]
        );

		$this->add_control(
            'freelancer_item_style',
            [
	...
	protected function render() {
		...
		if ( !empty($post) ) {
	        ?>
			<div class="freelancer-detail-related <?php echo esc_attr($el_class); ?>">
				<?php
				$tax_query = array();
				...
				if( $loop->have_posts() ):
                    ...
                    <div class="wrapper-services-related">
    		            <div class="widget-freelancers related-posts <?php echo esc_attr($layout_type.' item-'.$freelancer_item_style); ?> <?php echo esc_attr($el_class); ?>">
    		                    ...
    		                    <?php if ( $layout_type == 'carousel' ): ?>
    		                        <div class="slick-carousel <?php echo esc_attr($columns < $loop->post_count?'':'hidden-dots'); ?> <?php echo esc_attr($position_nav); ?>"
    		                            ...
    		                            <?php while ( $loop->have_posts() ): $loop->the_post(); ?>
    		                                <div class="item">
    		                                    <?php echo WP_Freeio_Template_Loader::get_template_part( 'freelancers-styles/inner-'. $freelancer_item_style ); ?>
    		                                </div>
    		                            <?php endwhile; ?>
    		                        </div>
    		                    <?php else: ?>
...
Elementor\Plugin::instance()->widgets_manager->register( new Freeio_Elementor_Widget_Detail_Freelancer_Related );

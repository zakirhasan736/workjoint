...
class Freeio_Elementor_Widget_Detail_Freelancer_Tags extends Elementor\Widget_Base {

	public function get_name() {
		return 'apus_element_detail_freelancer_tags';
	}

	public function get_title() {
		return esc_html__( 'Freelancer Details:: Tags', 'freeio' );
	}
	...
	protected function render() {
		...
		if ( !empty($post) ) {
	        ...
	        ?>
	        <div class="listing-detail-widget widget m-0">
		        <?php if ( $title ) { ?>
	                <h4 class="widget-title"><?php echo esc_html($title); ?></h4>
	            <?php } ?>
				<div class="freelancer-detail-tags <?php echo esc_attr($el_class); ?>">
					 <?php echo trim($tags); ?>
				</div>
			</div>
			<?php
...
Elementor\Plugin::instance()->widgets_manager->register( new Freeio_Elementor_Widget_Detail_Freelancer_Tags );

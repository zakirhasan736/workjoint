...
class Freeio_Elementor_Freeio_Services extends Elementor\Widget_Base {

	public function get_name() {
        return 'apus_element_services';
    }

	public function get_title() {
        return esc_html__( 'Apus Services', 'freeio' );
    }
	...
	protected function register_controls() {
        ...
        $this->start_controls_section(
            'content_section',
            [
                'label' => esc_html__( 'Services', 'freeio' ),
        ...
        foreach( $tax_keys as $tax_key ) {
            if ( $meta_obj->check_post_meta_exist($tax_key) ) {
                $this->add_control(
                    $tax_key.'_slugs',
                    [
                        'label' => sprintf(esc_html__( '%s Slug', 'freeio' ), $fields[WP_FREEIO_SERVICE_PREFIX.$tax_key]['name']),
                        'type' => Elementor\Controls_Manager::TEXTAREA,
                        'rows' => 1,
                        'default' => '',
        ...
        $this->add_control(
            'get_services_by',
            [
                'label' => esc_html__( 'Get Services By', 'freeio' ),
                'type' => Elementor\Controls_Manager::SELECT,
                'options' => array(
                    '' => esc_html__('Default', 'freeio'),
                    'featured' => esc_html__('Featured Services', 'freeio'),
                    'recent' => esc_html__('Recent Services', 'freeio'),
                ),
                'default' => ''
            ]
        );
        ...
        $this->add_control(
            'style_action',
            [
                'label' => esc_html__( 'Style Pagination, Navigation', 'freeio' ),
                'type' => Elementor\Controls_Manager::SELECT,
                'options' => array(
                    '' => esc_html__('Default', 'freeio'),
                    'st_white' => esc_html__('White', 'freeio'),
                ),
                'default' => '',
                'condition' => [
                    'layout_type' => 'carousel',
        ...
        $this->add_control(
            'view_color',
            [
                'label' => esc_html__( 'View More Color', 'freeio' ),
                'type' => Elementor\Controls_Manager::COLOR,
                'selectors' => [
	...
	protected function render() {
        ...
        $args = array(
            'limit' => $limit,
            'get_services_by' => $get_services_by,
            'orderby' => $orderby,
            'order' => $order
        );
            ...
            <div class="widget-services <?php echo esc_attr($el_class); ?>">
                ...
                <div class="widget-content">
                    <?php if ( $layout_type == 'carousel' ): ?>
                        <div class="slick-carousel <?php echo esc_attr($style_action); ?> <?php echo esc_attr( $fullscreen ? 'fullscreen' : '' ); ?>
                        <?php echo esc_attr( ( ($columns * $rows) >= ($loop->post_count)) ? 'hidden-dots':'' ); ?>
                        "
...
if ( version_compare(ELEMENTOR_VERSION, '3.5.0', '<') ) {
    Elementor\Plugin::instance()->widgets_manager->register_widget_type( new Freeio_Elementor_Freeio_Services );
} else {
    Elementor\Plugin::instance()->widgets_manager->register( new Freeio_Elementor_Freeio_Services );
}
...
class Freeio_Elementor_Service_Search_Form extends Elementor\Widget_Base {

	public function get_name() {
        return 'apus_element_service_search_form';
    }

	public function get_title() {
        return esc_html__( 'Apus Service Search Form', 'freeio' );
    }
	...
	protected function register_controls() {
        ...
        $fields = apply_filters( 'wp-freeio-default-service-filter-fields', array() );
	...
	protected function render() {
        ...
        $search_page_url = WP_Freeio_Mixes::get_services_page_url();

        freeio_load_select2();

        $filter_fields = apply_filters( 'wp-freeio-default-service-filter-fields', array() );
        $instance = array();
        ...
        ?>
            ...
            <button type="button" class="btn-search-header" data-bs-toggle="modal" data-bs-target="#search-header-<?php echo trim($widget_id); ?>"><i class="flaticon-loupe"></i></button>
            <div class="modal modal-search-header fade" id="search-header-<?php echo trim($widget_id); ?>" tabindex="-1" aria-hidden="true">
            ...
            foreach ($search_fields as $item) {
                ...
                if ( $item['filter_field'] == 'title' ) {
                    if ($item['enable_autocompleate_search']) {
                        ...
                        $filter_field['add_class'] = 'apus-autocompleate-input autocompleate-service';
                    }
...
if ( version_compare(ELEMENTOR_VERSION, '3.5.0', '<') ) {
    Elementor\Plugin::instance()->widgets_manager->register_widget_type( new Freeio_Elementor_Service_Search_Form );
} else {
    Elementor\Plugin::instance()->widgets_manager->register( new Freeio_Elementor_Service_Search_Form );
}
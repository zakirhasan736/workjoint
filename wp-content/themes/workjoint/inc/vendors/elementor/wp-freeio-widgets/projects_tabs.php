...
class Freeio_Elementor_Freeio_Projects_Tabs extends Elementor\Widget_Base {

	public function get_name() {
        return 'apus_element_projects_tabs';
    }

	public function get_title() {
        return esc_html__( 'Apus Projects Tabs', 'freeio' );
    }
    ...
    public function get_tax_keys() {
        return array('category', 'location', 'skill');
    }
	...
	protected function register_controls() {
        ...
        $this->start_controls_section(
            'content_section',
            [
                'label' => esc_html__( 'Projects', 'freeio' ),
        ...
        foreach( $tax_keys as $tax_key ) {
            if ( $meta_obj->check_post_meta_exist($tax_key) ) {
                $repeater->add_control(
                    $tax_key.'_slugs',
                    [
                        'label' => sprintf(esc_html__( '%s Slug', 'freeio' ), $fields[WP_FREEIO_PROJECT_PREFIX.$tax_key]['name']),
        ...
        $repeater->add_control(
            'get_projects_by',
            [
                'label' => esc_html__( 'Get Projects By', 'freeio' ),
                'type' => Elementor\Controls_Manager::SELECT,
                'options' => array(
                    '' => esc_html__('Default', 'freeio'),
                    'featured' => esc_html__('Featured Projects', 'freeio'),
                    'recent' => esc_html__('Recent Projects', 'freeio'),
                ),
                'default' => 'recent'
            ]
        );
        ...
        $this->add_control(
            ...
            [
                ...
                'placeholder' => esc_html__( 'Enter your listing tabs here', 'freeio' ),
        ...
        $this->add_control(
            ...
            [
                ...
                'input_type' => 'number',
                'description' => esc_html__( 'Limit projects to display', 'freeio' ),
                'default' => 4
            ]
        );
	...
	protected function render() {
        ...
        ?>
        <div class="widget-projects-tabs <?php echo esc_attr($layout_type); ?> <?php echo esc_attr($el_class); ?>">
            ...
            <div class="tab-content">
                <?php
                    ...
                    <div id="tab-<?php echo esc_attr($_id);?>-<?php echo esc_attr($tab_count); ?>" class="tab-pane fade <?php echo esc_attr($tab_count == 0 ? 'active show' : ''); ?>">
                        ...
                        $args = array(
                            'limit' => $limit,
                            'get_projects_by' => !empty($tab['get_projects_by']) ? $tab['get_projects_by'] : 'recent',
                            ...
                            <?php if ( $layout_type == 'carousel' ): ?>
                                <div class="slick-carousel <?php echo esc_attr( ( ($columns * $rows) >= ($loop->post_count)) ? 'hidden-dots':'' ); ?>"
                                    ...
                                    <?php while ( $loop->have_posts() ): $loop->the_post(); ?>
                                        <div class="item">
                                            <?php echo WP_Freeio_Template_Loader::get_template_part( 'projects-styles/inner-'.$project_item_style ); ?>
                                        </div>
                                    <?php endwhile; ?>
                                </div>
                            <?php else: ?>
                                ...
                                <div class="row">
                                    <?php while ( $loop->have_posts() ) : $loop->the_post();
                                    ?>
                                        <div class="col-lg-<?php echo esc_attr($mdcol); ?> col-md-<?php echo esc_attr($smcol); ?> col-<?php echo esc_attr( $xscol ); ?> <?php echo esc_attr($classes); ?>">
                                            <?php echo WP_Freeio_Template_Loader::get_template_part( 'projects-styles/inner-'.$project_item_style ); ?>
                                        </div>
                                    <?php endwhile; ?>
                                </div>
...
if ( version_compare(ELEMENTOR_VERSION, '3.5.0', '<') ) {
    Elementor\Plugin::instance()->widgets_manager->register_widget_type( new Freeio_Elementor_Freeio_Projects_Tabs );
} else {
    Elementor\Plugin::instance()->widgets_manager->register( new Freeio_Elementor_Freeio_Projects_Tabs );
}
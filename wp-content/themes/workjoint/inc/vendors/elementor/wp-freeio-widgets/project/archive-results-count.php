...
class Freeio_Elementor_Widget_Project_Archive_Results_Count extends Elementor\Widget_Base {

	public function get_name() {
		return 'apus_element_project_archive_results_count';
	}

	public function get_title() {
		return esc_html__( 'Project Archive:: Results Count', 'freeio' );
	}
	...
	protected function render() {
        ...
        if ( !empty($freeio_projects) ) {
	        $total = $freeio_projects->found_posts;
			$per_page = $freeio_projects->query_vars['posts_per_page'];
			$current = max( 1, $freeio_projects->get( 'paged', 1 ) );
		} else {
...
Elementor\Plugin::instance()->widgets_manager->register( new Freeio_Elementor_Widget_Project_Archive_Results_Count );

...
class Freeio_Elementor_Widget_Detail_Employer_Overview extends Elementor\Widget_Base {

	public function get_name() {
		return 'apus_element_detail_employer_overview';
	}

	public function get_title() {
		return esc_html__( 'Employer Details:: Overview', 'freeio' );
	}
	...
	public function get_employer_fields() {
		if ( method_exists('WP_Freeio_Custom_Fields', 'get_all_custom_fields') ) {
			$fields = WP_Freeio_Custom_Fields::get_all_custom_fields(array(), 'front', WP_FREEIO_EMPLOYER_PREFIX);
		} else {
			$fields = WP_Freeio_Custom_Fields::get_custom_fields(array(), 'front', 0, WP_FREEIO_EMPLOYER_PREFIX);
		}
	...
	protected function register_controls() {
		...
		$all_fields = $this->get_employer_fields();
        ...
        $repeater->add_control(
            ...
            [
                ...
                'default'       => true,
                'condition' => [
                    'show_field' => array('_employer_category', '_employer_location'),
                ],
            ]
        );
	...
	protected function render() {
		...
		if ( !empty($post_id) && !empty($items) ) {
			$all_fields = $this->get_employer_fields();
			?>
			<div class="widget m-0 listing-detail-widget">
				<?php if ( $title ) { ?>
	                <h4 class="widget-title"><?php echo esc_html($title); ?></h4>
	            <?php } ?>
				<div class="employer-detail-overview <?php echo esc_attr($el_class); ?>">
					<ul class="list-employer-info">
						<?php foreach ($items as $item) {
							$show_field = true;
							if ( $item['show_field'] == '_employer_email' ) {
								$email = WP_Freeio_Employer::get_display_email( $post_id );
								if ( !$email ) {
									$show_field = false;
								}
							} elseif ( $item['show_field'] == '_employer_phone' ) {
								$phone = WP_Freeio_Employer::get_display_phone( $post_id );
								if ( !$phone ) {
									$show_field = false;
								}
							}
	...
	private function render_meta_field($item, $post_id, $field_name) {
		...
		$value_html = '';
		$taxs = array('_employer_category', '_employer_location');
		$taxs_real = array('_employer_category' => 'employer_category', '_employer_location' => 'location');
			...
			if ( $meta_obj->check_custom_post_meta_exist($show_field) ) {
				...
				if ( $tax_values && ! is_wp_error( $tax_values ) ) {
					...
					foreach ($tax_values as $term) {
						if ( $show_url ) {
						?>
			            	<a class="employer-tax" href="<?php echo esc_url(get_term_link($term)); ?>"><?php echo esc_html($term->name); ?></a><?php if($number < count($tax_values)) echo trim(', ');?>
			        	<?php
			        	} else {
			        		?>
			        		<span class="employer-tax"><?php echo esc_html($term->name); ?></span><?php if($number < count($tax_values)) echo trim(', ');?>
			        		<?php
			        	}
			        	$number++;
			    	}
...
Elementor\Plugin::instance()->widgets_manager->register( new Freeio_Elementor_Widget_Detail_Employer_Overview );

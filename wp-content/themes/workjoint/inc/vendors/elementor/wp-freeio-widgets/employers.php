...
class Freeio_Elementor_Freeio_Employers extends Elementor\Widget_Base {

	public function get_name() {
        return 'apus_element_freeio_employers';
    }

	public function get_title() {
        return esc_html__( 'Apus Employers', 'freeio' );
    }
	...
	protected function register_controls() {
        ...
        $this->start_controls_section(
            'content_section',
            [
                'label' => esc_html__( 'Employers', 'freeio' ),
        ...
        foreach( $tax_keys as $tax_key ) {
            if ( $meta_obj->check_post_meta_exist($tax_key) ) {
                $this->add_control(
                    $tax_key.'_slugs',
                    [
                        'label' => sprintf(esc_html__( '%s Slug', 'freeio' ), $fields[WP_FREEIO_EMPLOYER_PREFIX.$tax_key]['name']),
                        'type' => Elementor\Controls_Manager::TEXTAREA,
                        'rows' => 1,
                        'default' => '',
        ...
        $this->add_control(
            'get_employers_by',
            [
                'label' => esc_html__( 'Get Employers By', 'freeio' ),
                'type' => Elementor\Controls_Manager::SELECT,
                'options' => array(
                    'featured' => esc_html__('Featured Employers', 'freeio'),
                    'recent' => esc_html__('Recent Employers', 'freeio'),
                ),
                'default' => 'recent'
            ]
        );
...
if ( version_compare(ELEMENTOR_VERSION, '3.5.0', '<') ) {
    Elementor\Plugin::instance()->widgets_manager->register_widget_type( new Freeio_Elementor_Freeio_Employers );
} else {
    Elementor\Plugin::instance()->widgets_manager->register( new Freeio_Elementor_Freeio_Employers );
}
...
class Freeio_Elementor_Widget_Service_Archive_Results_Count extends Elementor\Widget_Base {

	public function get_name() {
		return 'apus_element_service_archive_results_count';
	}

	public function get_title() {
		return esc_html__( 'Service Archive:: Results Count', 'freeio' );
	}
	...
	protected function render() {
        ...
        if ( !empty($freeio_services) ) {
	        $total = $freeio_services->found_posts;
			$per_page = $freeio_services->query_vars['posts_per_page'];
			$current = max( 1, $freeio_services->get( 'paged', 1 ) );
		} else {
...
Elementor\Plugin::instance()->widgets_manager->register( new Freeio_Elementor_Widget_Service_Archive_Results_Count );

...
class Freeio_Elementor_Widget_Detail_Job_Related extends Elementor\Widget_Base {

	public function get_name() {
		return 'apus_element_detail_job_related';
	}

	public function get_title() {
		return esc_html__( 'Job Details:: Related', 'freeio' );
	}
	...
	protected function register_controls() {
		$this->start_controls_section(
            'content_section',
            [
                'label' => esc_html__( 'Properties', 'freeio' ),
		...
		if ( !empty($post) ) {
	        ?>
				...
				if( $loop->have_posts() ):
		            ...
		            ?>
                    <div class="wrapper-services-related related-detail-listing <?php echo esc_attr($el_class); ?>">
    		            <div class="related-posts <?php echo esc_attr($layout_type.' item-'.$job_item_style); ?> <?php echo esc_attr($el_class); ?>">
    	                    <?php if ( $title ) { ?>
    		                    ...
    		                    <?php if ( $layout_type == 'carousel' ): ?>
    		                        <div class="slick-carousel <?php echo esc_attr($columns < $loop->post_count?'':'hidden-dots'); ?> <?php echo esc_attr($position_nav); ?>"
    		                            ...
    		                            <?php while ( $loop->have_posts() ): $loop->the_post(); ?>
    		                                <div class="item">
    		                                    <?php echo WP_Freeio_Template_Loader::get_template_part( 'jobs-styles/inner-'. $job_item_style ); ?>
    		                                </div>
    		                            <?php endwhile; ?>
    		                        </div>
    		                    <?php else: ?>
    		                        ...
    		                        <div class="row">
    		                            <?php while ( $loop->have_posts() ) : $loop->the_post(); ?>
    		                                <div class="jobs-item-related col-xl-<?php echo esc_attr($mdcol); ?> col-tablet-extra-<?php echo esc_attr($columns_tablet_extra); ?> col-md-<?php echo esc_attr($smcol); ?> col-<?php echo esc_attr( $xscol ); ?> list-item">
    		                                    <?php echo WP_Freeio_Template_Loader::get_template_part( 'jobs-styles/inner-'. $job_item_style ); ?>
    		                                </div>
    		                            <?php endwhile; ?>
    		                        </div>
...
Elementor\Plugin::instance()->widgets_manager->register( new Freeio_Elementor_Widget_Detail_Job_Related );

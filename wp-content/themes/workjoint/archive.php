...
?>
<section id="main-container" class="main-content  <?php echo apply_filters('freeio_blog_content_class', 'container');?> inner">
	<?php ( (empty($sidebar_configs['left']) && empty($sidebar_configs['right'])) || (!empty($_GET['style']) && $_GET['style'] =='gridfull') )?'': freeio_before_content( $sidebar_configs ); ?>
	<div class="row responsive-medium">
		...
		<div id="main-content" class="col-12 <?php echo esc_attr($sidebar_configs['main']['class']); ?>">
			...
			<?php if ( have_posts() ) : ?>
				...
				<?php
				if ( (empty($sidebar_configs['left']) && empty($sidebar_configs['right']) && freeio_get_config('blog_archive_top_categories', false)) || (!empty($_GET['style']) && $_GET['style'] =='gridfull') )	{
				?>

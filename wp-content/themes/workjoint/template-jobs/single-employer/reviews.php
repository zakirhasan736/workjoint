...
<div id="reviews">
	...
	<?php if ( have_comments() ) : ?>
		<h3 class="title"><?php comments_number( esc_html__('0 Comments', 'freeio'), esc_html__('1 Comment', 'freeio'), esc_html__('% Comments', 'freeio') ); ?></h3>
	<?php endif; ?>
	<div id="comments">
		<?php if ( have_comments() ) : ?>

			<ol class="comment-list">
				<?php wp_list_comments( array( 'callback' => array( 'WP_Freeio_Review', 'employer_comments' ) ) ); ?>
			</ol>
			...
			<div id="review_form">
				<?php
					$comment_form = array(
						...
						'comment_notes_after'  => '',
						'fields'               => array(
							'author' => '<div class="row"><div class="col-xs-12 col-sm-12"><div class="form-group"><label>'.esc_html__( 'Name', 'freeio' ).'</label>'.
							            '<input id="author" placeholder="'.esc_attr__( 'Your Name', 'freeio' ).'" class="form-control" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" aria-required="true" /></div></div>',
							'email'  => '<div class="col-xs-12 col-sm-12"><div class="form-group"><label>'.esc_html__( 'Email', 'freeio' ).'</label>' .
							            '<input id="email" placeholder="'.esc_attr__( 'your@mail.com', 'freeio' ).'" class="form-control" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30" aria-required="true" /></div></div></div>',
						),
					...
					$comment_form['comment_field'] .= '<div class="form-group space-30"><label>'.esc_html__( 'Review', 'freeio' ).'</label><textarea id="comment" class="form-control" placeholder="'.esc_attr__( 'Write Comment', 'freeio' ).'" name="comment" cols="45" rows="5" aria-required="true" required></textarea></div>';

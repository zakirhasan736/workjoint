jQuery(document).ready(function($){
    ...
    // loading admin icon
    $("body").on("wp_freeio_all_loading_icons", function(){
        freeio_get_awesome_icons();
        freeio_get_flaticon_icons();
        freeio_get_themify_icons();
    });

    function freeio_get_awesome_icons() {
        ...
        wp_freeio_all_loaded_icons.fontwesome = icons_load_call;
    }

    function freeio_get_flaticon_icons() {
        var files = document.querySelectorAll('*[id^="flaticon"]'), html = '', css;
        ...
        for( var i=0; i < files.length; i++ ){
            css = Array.prototype.map.call( files[i].sheet.cssRules, freeio_css_text ).join('\n');
        ...
        wp_freeio_all_loaded_icons.flaticon = icons_load_call;
    }

    function freeio_get_themify_icons() {
        var files = document.querySelectorAll('*[id^="themify-icons"]'), html = '', css;
        ...
        wp_freeio_all_loaded_icons.themify = icons_load_call;
    }

    function freeio_css_text(x) {
        return x.cssText;
    }
    ...
    $(document).on('change', '#_service_layout_template', function(){
        var val = $(this).val();
        show_hide_service_layout_setting(val);
    });
    setTimeout(function() {
        if ( $('#_service_layout_template').length > 0 ) {
            var val = $('#_service_layout_template').val();
            show_hide_service_layout_setting(val);
        }
    }, 100);
    function show_hide_service_layout_setting(val) {
        if ( val == 'default' ) {
            $('.cmb2-id--service-layout-type').show();
            $('.cmb2-id--service-elementor-template').hide();
        } else if( val == 'elementor' ) {
            $('.cmb2-id--service-layout-type').hide();
            $('.cmb2-id--service-elementor-template').show();
        } else {
            $('.cmb2-id--service-layout-type').hide();
            $('.cmb2-id--service-elementor-template').hide();
        }
    ...
    $(document).on('change', '#_project_layout_template', function(){
        var val = $(this).val();
        show_hide_project_layout_setting(val);
    });
    setTimeout(function() {
        if ( $('#_project_layout_template').length > 0 ) {
            var val = $('#_project_layout_template').val();
            show_hide_project_layout_setting(val);
        }
    }, 100);
    function show_hide_project_layout_setting(val) {
        if ( val == 'default' ) {
            $('.cmb2-id--project-layout-type').show();
            $('.cmb2-id--project-elementor-template').hide();
        } else if( val == 'elementor' ) {
            $('.cmb2-id--project-layout-type').hide();
            $('.cmb2-id--project-elementor-template').show();
        } else {
            $('.cmb2-id--project-layout-type').hide();
            $('.cmb2-id--project-elementor-template').hide();
        }
    ...
    // freelancer single layout
    $(document).on('change', '#_freelancer_layout_template', function(){
        var val = $(this).val();
        show_hide_freelancer_layout_setting(val);
    });
    setTimeout(function() {
        if ( $('#_freelancer_layout_template').length > 0 ) {
            var val = $('#_freelancer_layout_template').val();
            show_hide_freelancer_layout_setting(val);
        }
    }, 100);
    function show_hide_freelancer_layout_setting(val) {
        if ( val == 'default' ) {
            $('.cmb2-id--freelancer-layout-type').show();
            $('.cmb2-id--freelancer-elementor-template').hide();
        } else if( val == 'elementor' ) {
            $('.cmb2-id--freelancer-layout-type').hide();
            $('.cmb2-id--freelancer-elementor-template').show();
        } else {
            $('.cmb2-id--freelancer-layout-type').hide();
            $('.cmb2-id--freelancer-elementor-template').hide();
        }

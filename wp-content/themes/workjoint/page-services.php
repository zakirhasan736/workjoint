...
if ( isset( $_REQUEST['load_type'] ) && WP_Freeio_Mixes::is_ajax_request() ) {
	...
	if ( !empty($wp_query->post->post_content) ) {
		$shortcode_atts = freeio_get_shortcode_atts($wp_query->post->post_content, 'wp_freeio_services');
	...
	?>
		<section id="main-container" class="page-service-board inner layout-type-<?php echo esc_attr($layout_type); ?> <?php echo esc_attr($top_content ? 'has-filter-top':''); ?>">
			...
			<div class="layout-service-sidebar main-content <?php echo apply_filters('freeio_page_content_class', 'container');?> inner">

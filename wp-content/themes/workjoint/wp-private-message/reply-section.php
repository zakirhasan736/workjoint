...
?>
  <div class="recipient-info clearfix">
    <div class="message-item float-start">
        <?php
          $display_name = get_the_author_meta('display_name', $recipient_id);
          $display_name = apply_filters('freeio-private-message-user-display-name', $display_name, $recipient_id);
        ?>
        <div class="avatar">
          <?php freeio_user_avarta( $recipient_id ); ?>
        </div>
        <div class="content">
    ...
    <a href="javascript:void(0);" class="delete-message-btn float-end" data-id="<?php echo esc_attr($selected_post->ID); ?>" data-nonce="<?php echo esc_attr(wp_create_nonce( 'wp-private-message-delete-message-nonce' )); ?>"><?php esc_html_e('Delete Conversation', 'freeio'); ?></a>
  </div>
    ...
    <div class="list-replies-inner">
      ...
      if ( $next_page <= $reply_messages->max_num_pages ) { ?>
        <div class="loadmore-action">
          <a href="javascript:void(0);" class="loadmore-replied-btn" data-paged="<?php echo esc_attr($next_page); ?>" data-parent="<?php echo esc_attr($selected_post->ID); ?>"><?php esc_html_e( 'Load more', 'freeio' ); ?></a>
        </div>
      <?php } ?>
